...
public:
    ...
    */
    this(string file) {
        ...
        enforce( image.e == 0, "%s: %s".format(file, IF_ERROR[image.e]));
    ...
    /**
        ...
        * JPEG baseline
    */
    this(ubyte[] data, string name) {

        // Load image from disk, as RGBA 8-bit
        IFImage image = read_image(data, 4, 8);
        enforce( image.e == 0, "%s: %s".format(name, IF_ERROR[image.e]));
        scope(exit) image.free();
...
public:
    ...
    */
    this(string file) {
        ...
        this(image.buf8, image.w, image.h, 4, 4);
    }
    ...
    */
    this(ShallowTexture shallow) {
        this(shallow.data, shallow.width, shallow.height, 4, 4);
    }

    /**
        Creates a new empty texture
    */
    this(int width, int height, int channels = 4, int alignment = 4) {
        ...
        this(empty, width, height, channels, alignment);
    }

    /**
        Creates a new texture from specified data
    */
    this(ubyte[] data, int width, int height, int channels = 4, int alignment = 4) {
